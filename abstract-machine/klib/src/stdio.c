...
#define PUTCHAR(ch)                      \
    ...
    int length_modifier = 0; // length_modifier==1 when ld, lu...
    // int base_decorator = 0; // base_decorator==1 when #...
    int i_fmt = 0, i_out = 0;
    while (fmt[i_fmt])
    {
        /*
        ...
        */
        ...
        if (fmt[i_fmt] == '%')
        {
            ...
            // 4. Handle Conversion Specifier
            // !!! Remember to break after each case !!!
            case 'd':
            {
                int64_t d = (length_modifier == 1) ? va_arg(ap, int64_t) : (int64_t)va_arg(ap, int32_t);
                char buf[100];
                int neg = 0, i_buf = 0;
            ...
            case 'x': // print as unsigned 
            {
                uint64_t d = (length_modifier == 1) ? va_arg(ap, uint64_t) : (uint64_t)va_arg(ap, uint32_t);
                char buf[100];
                int i_buf = 0;
                if (d == 0)
                {
